...
export default function App() {
  ...
  const handleFormat = () => {
    ...
    let formatted = code.trim().replace(/\s*{\s*/g, ' {\n ').replace(/\s*;\s*/g, ';\n').replace(/\s*}\s*/g, '\n}\n').replace(/\n\s*\n/g, '\n');
    setCode(formatted);
  };
    ...
    <div className={`app-wrapper ${theme}-theme ${lang === 'ar'? 'rtl-dir' : 'ltr-dir'}`}>
      <style>{`
       ...
       .app-wrapper {
          ...
          padding: 20px 10px; font-family: 'Cairo', sans-serif;
          background-size: 400% 400%; animation: dynamicGradient 15s ease infinite;
        }
       .dark-theme { background: linear-gradient(-45deg, #0f172a, #1e1b4b, #311042, #0f172a); color: #e2e8f0; }
       .light-theme { background: linear-gradient(-45deg, #f8fafc, #e0f2fe, #f3e8ff, #f8fafc); color: #1e293b; }
       .rtl-dir { direction: rtl; }.ltr-dir { direction: ltr; }
       ...
       .light-theme pre { background: rgba(241, 245, 249, 0.9); }
        ...
        header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; border-bottom: 1px solid rgba(148, 163, 184, 0.2); padding-bottom: 20px; flex-wrap: wrap; gap: 15px; }
        header h1 { font-size: 2rem; color: #38bdf8; font-weight: 800; }
       .controls { display: flex; gap: 10px; align-items: center; flex-wrap: wrap; }
       .toggle-btn,.dropdown-trigger { background: #38bdf8; color: #0f172a; border: none; padding: 8px 16px; border-radius: 20px; font-weight: bold; cursor: pointer; font-size: 0.9rem; height: 38px; display:flex; align-items:center; }

       .custom-dropdown-container { position: relative; }
       .dropdown-menu { position: absolute; top: 45px; right: 0; background: #1e293b; border: 1px solid #334155; border-radius: 12px; box-shadow: 0 10px 25px rgba(0, 0, 0, 0.3); z-index: 1000; width: 220px; padding: 8px; display: flex; flex-direction: column; gap: 6px; }
       .dropdown-search { width: 100%; padding: 6px 10px; border-radius: 6px; border: 1px solid #475569; background: #0f172a; color: #e2e8f0; font-size: 0.85rem; }
       .dropdown-items-list { max-height: 200px; overflow-y: auto; }
       .dropdown-item { padding: 12px 16px; border-radius: 6px; cursor: pointer; font-size: 1rem; display: flex; align-items: center; gap: 8px; color: #e2e8f0; min-height: 44px; }
       .dropdown-item:hover { background: #334155; }
       .dropdown-item.selected { background: #38bdf8; color: #0f172a; }

       .tabs { display: flex; gap: 10px; margin-bottom: 25px; background: rgba(148, 163, 184, 0.1); padding: 6px; border-radius: 12px; flex-wrap: wrap; }
       .light-theme.tabs{ background: rgba(0,0,0,0.05); }
       .tab-button { background: none; border: none; padding: 10px 20px; border-radius: 8px; font-size: 1rem; font-weight: bold; color: #94a3b8; cursor: pointer; flex: 1; min-width: 120px; text-align: center; }
       .tab-button.active { background: #38bdf8; color: #0f172a; }

       .workspace { display: grid; grid-template-columns: 1fr 1.2fr; gap: 30px; }
       .card { border: 1px solid; border-radius: 16px; padding: 24px; width: 100%; overflow-wrap:break-word; }
       .form-group { margin-bottom: 20px; }
       .form-group label { display: block; margin-bottom: 8px; font-weight: 600; opacity: 0.8; }
        textarea { width: 100%; border: 1px solid; border-radius: 8px; padding: 12px; font-family: 'Fira Code', monospace; font-size: 0.95rem; resize: vertical; }
       .btn-group { display: flex; gap: 10px; flex-wrap: wrap; }
       .btn { flex: 1; padding: 12px; border: none; font-weight: bold; border-radius: 8px; font-size: 1rem; min-height:44px; cursor: pointer; }
       .btn-primary { background: #38bdf8; color: #0f172a; }
       .btn-secondary { background: #475569; color: #f1f5f9; }
       .btn-accent { background: #10b981; color: white; }

       .diagnosis-header { border-bottom: 2px solid rgba(148, 163, 184, 0.2); padding-bottom: 15px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; }
       ...
       .code-block-title { font-size: 0.9rem; opacity: 0.7; margin-bottom: 6px; }
        pre { padding: 15px; border-radius: 8px; overflow-x: auto; max-width: 100%; font-family: 'Fira Code', monospace; font-size: 14px; margin-bottom: 20px; white-space: pre-wrap; word-wrap: break-word; }
       ...
       .copy-btn { background: #10b981; color: white; border: none; padding: 8px 12px; border-radius: 6px; font-size: 0.9rem; cursor: pointer; font-weight: bold; min-height:36px; min-width: 70px; }
        ...
        @media(max-width: 768px){
         ...
         .tab-button { width: 100%; min-height: 44px; }
          textarea { font-size: 16px; min-height: 120px; }
         .btn-group { flex-direction: column; }
         .btn,.dropdown-trigger { width: 100%; min-height: 44px; font-size: 16px; padding: 12px; }
         .dropdown-menu { width: 100%; max-width: 100%; }
         .dropdown-search { font-size: 16px; }
      ...
      <div className="doctor-container">
        <header>
          ...
          <div className="controls">
            <div className="custom-dropdown-container" ref={dropdownRef}>
              <button className="dropdown-trigger" onClick={() => setIsLangDropdownOpen(!isLangDropdownOpen)}>
                <span>🌐 {currentLangData.flag} {currentLangData.nativeName}</span>
              </button>
              {isLangDropdownOpen && (
                <div className="dropdown-menu">
                  <input type="text" className="dropdown-search" placeholder={text.searchPlaceholder} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} autoFocus />
                  <div className="dropdown-items-list">
                    {filteredLanguages.map((item) => (
                      <div key={item.code} className={`dropdown-item ${item.code === lang? 'selected' : ''}`} onClick={() => handleLangSelect(item.code)}>
                        <span>{item.flag}</span><span>{item.nativeName}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="tabs">
          <button className={`tab-button ${activeTab === 'doctor'? 'active' : ''}`} onClick={() => { setActiveTab('doctor'); setDiagnosis(null); }}>
            {text.tabDoctor}
          </button>
          <button className={`tab-button ${activeTab === 'optimizer'? 'active' : ''}`} onClick={() => { setActiveTab('optimizer'); setDiagnosis(null); }}>
            {text.tabOptimizer}
          </button>
        </div>
          ...
          <div className="card">
            {activeTab === 'doctor' && (
              <div className="form-group">
                ...
                <textarea rows="3" placeholder={text.errorPlaceholder} value={errorMessage} onChange={(e) => setErrorMessage(e.target.value)} />
              </div>
            )}
            <div className="form-group">
              <label>{text.codeLabel}</label>
              <textarea rows="8" placeholder={text.codePlaceholder} value={code} onChange={(e) => setCode(e.target.value)} />
            </div>
            ...
            {diagnosis? (
              <div>
                <div className="diagnosis-header">
                  ...
                  <button className="copy-btn" onClick={copyToClipboard}>{copied? text.copiedText : text.copyBtn}</button>
                </div>
                ...
                <div className="tip-box"><strong>{text.tipTitle}</strong><p style={{ marginTop: '5px', color: theme === 'dark'? '#f1f5f9' : '#1e293b' }}>{diagnosis.tip}</p></div>
              </div>
            ) : (
              <div style={{ color: '#64748b' }}>
                <span style={{ fontSize: '3.5rem' }}>{activeTab === 'doctor'? '🔬' : '⚡'}</span>
                <p style={{ marginTop: '15px', font-size: '1.1rem' }}>{activeTab === 'doctor'? text.placeholderDoctor : text.placeholderOptimizer}</p>
              </div>
            )}
          </div>
        </div>
